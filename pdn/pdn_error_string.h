...
namespace pdn
{
	...
	inline error_msg_string reinterpret_to_err_msg_str(const char* begin, const char* end)
	{
		return { reinterpret_cast<const error_msg_char*>(begin), reinterpret_cast<const error_msg_char*>(end) };
	}

	inline error_msg_string reinterpret_to_err_msg_str(const ::std::string_view s)
	{
