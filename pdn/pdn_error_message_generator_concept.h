...
namespace pdn::concepts
{
	...
	{
		{ err_msg_gen.generate_error_message(errc_variant, err_msg_str) } -> ::std::convertible_to<error_msg_string>;
		{ err_msg_gen.generate_error_message(::std::move(errc_variant), ::std::move(err_msg_str)) } -> ::std::convertible_to<error_msg_string>;
	};
}
