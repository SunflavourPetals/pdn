...
namespace pdn
{
	template <unicode::concepts::code_unit char_t, concepts::function_package_for_lexer function_package>
	class lexer;
}
...
namespace pdn::detail
{
	constexpr bool is_non_token_state(dfa_state_code c) noexcept
	{
	...
	constexpr bool is_token_state(dfa_state_code c) noexcept
	{
		return !is_non_token_state(c);
	}

	inline bool check_no_0b_prefix(const ::std::string& seq)
	{
		return seq.size() < 2 || seq[0] != '0' || (seq[1] != 'b' && seq[1] != 'B');
	}

	inline bool check_0b_prefix(const ::std::string& seq)
	{
		return !check_no_0b_prefix(seq);
	}

	inline bool check_no_0x_prefix(const ::std::string& seq)
	{
		return seq.size() < 2 || seq[0] != '0' || (seq[1] != 'x' && seq[1] != 'X');
	}

	inline bool check_0x_prefix(const ::std::string& seq)
	{
		return !check_no_0x_prefix(seq);
	}

	inline bool check_from_chars_succeed(const ::std::from_chars_result from_chars_r, const char* expected)
	{
		return (from_chars_r.ec == ::std::errc{}) && (from_chars_r.ptr == expected);
	}

	inline void remove_all_separator(::std::string& num_str)
	{
		::std::erase(num_str, '\'');
	}

	inline auto to_raw_err_str(error_msg_string err_msg_str) -> raw_error_message_type::error_string
	{
		return raw_error_message_type::error_string{ ::std::move(err_msg_str) };
	}

	template <typename char_t>
	inline auto to_err_ms(::std::basic_string_view<char_t> s) -> error_msg_string
	{
		if constexpr (::std::same_as<char_t, error_msg_char>)
		{
			return error_msg_string(::std::cbegin(s), ::std::cend(s));
		}
		else
		{
	...
	inline auto get_esc_not_scalar_err_msg(error_msg_string      escape_sign, // e.g. "u" means "\u"
	...
	inline void get_numeric_escape_sequence(::std::string& sequence,
		                                             auto& begin,
		                                             auto  end,
	...
	inline auto get_numeric_escape_sequence(::std::size_t  count,
		                                    ::std::string& sequence,
		                                             auto& begin,
		                                             auto  end,
	...
	inline void process_from_chars_error(const ::std::from_chars_result            from_chars_r,
		                                 ...
		                                 auto                                      helper)
	{
		...
		assert(unresolved_char_ptr >= src.data() && unresolved_char_ptr <= (src.data() + src.size()) && "invalid arguments");
		...
		if (err_code == ::std::errc{})
		{
			if (unresolved_char_ptr != src.data() + src.size())
			{
				...
				helper.post_err(pos, ec, raw_err{ num_seq_to_ems(), ptr_diff, err_code, lit_type });
			}
		}
		else
		{
	...
	inline bool get_escape(auto& oc,    // out param, the result of get escape
		                   auto& begin, // pointing first char witch after '\'
		                   auto  end,
		                   bool  enable_escape_back_quote, // is enable escape sequence \`
		                   auto  helper)
	{
		using escape_value_t = ::std::uint_least32_t;

		auto position = helper.get_pos();

		using enum lexical_error_code;
		if (begin == end)
		{
			helper.post_err(position, escape_error_unknown_escape_sequence, to_raw_err_str(u8""_em));
			return false;
		}
		...
		case U'o': // \o{n...}
		{
			...
			{
				helper.post_err(position,
				                escape_error_o_not_followed_by_left_brackets,
				                to_raw_err_str(u8"o"_em));
				return false;
			}
			c = *begin;
			++begin; // -> n (oct)
			...
			{
				helper.post_err(position,
				                escape_error_o_not_terminated_with_right_brackets,
				                to_raw_err_str(u8"o{"_em + seq_to_err_msg_str()));

				if (sequence.empty())
				{
					helper.post_err(position,
					                escape_error_o_empty_delimited_escape_sequence,
					                to_raw_err_str(u8"o{"_em));
				}
				return false;
			}
			...
			if (!check_from_chars_succeed(from_chars_r, sequence.data() + sequence.size())) [[unlikely]]
			{
				...
				process_from_chars_error(from_chars_r, sequence, position, oct_integer, true, helper);
				rt_flag = true;
			}
			if (!unicode::is_scalar_value(parse_val)) [[unlikely]]
			{
				helper.post_err(position,
				                ...
				                get_esc_not_scalar_err_msg(u8"o"_em, seq_to_err_msg_str(), parse_val, true));
				rt_flag = true;
			}
			if (rt_flag) [[unlikely]]
			{
		...
		case U'x': // \xn... \x{n...}
		{
			...
			{
				helper.post_err(position,
				                escape_error_x_used_with_no_following_hex_digits,
				                to_raw_err_str(u8"x"_em));
				return false;
			}
			c = *begin;
			if (c == U'{')
			...
			if (is_with_curly_brackets)
			{
				...
				{
					helper.post_err(position,
					                escape_error_x_not_terminated_with_right_brackets,
					                to_raw_err_str(u8"x{"_em + seq_to_err_msg_str()));
					if (sequence.empty())
					{
						helper.post_err(position,
						                escape_error_x_empty_delimited_escape_sequence,
						                to_raw_err_str(u8"x{"_em));
					}
					return false;
				}
			...
			{
				using enum raw_error_message_type::number_type;
				process_from_chars_error(from_chars_r, sequence, position, hex_integer, true, helper);
				rt_flag = true;
			}
			if (!unicode::is_scalar_value(parse_val)) [[unlikely]]
			{
		...
		case U'u': // \unnnn \u{n...} Universal character names
		{
			...
			{
				helper.post_err(position,
				                escape_error_u_incomplete_universal_character_name,
				                to_raw_err_str(u8"u"_em));
				return false;
			}
			c = *begin;
			if (c == U'{')
			...
			if (is_with_curly_brackets)
			{
				...
				{
					helper.post_err(position,
					                escape_error_u_not_terminated_with_right_brackets,
					                to_raw_err_str(u8"u{"_em + seq_to_err_msg_str()));
					if (sequence.empty())
					{
						helper.post_err(position,
						                escape_error_u_empty_delimited_escape_sequence,
						                to_raw_err_str(u8"u{"_em));
					}
					return false;
				}
			...
			if (!unicode::is_scalar_value(parse_val)) [[unlikely]]
			{
				helper.post_err(position,
				                escape_error_not_unicode_scalar_value,
				                get_esc_not_scalar_err_msg(u8"u"_em, seq_to_err_msg_str(), parse_val, is_with_curly_brackets));
				rt_flag = true;
			}
			if (rt_flag) [[unlikely]]
			{
			...
			// \Unnnnnnnn Universal character names
		{
			...
			if (!unicode::is_scalar_value(parse_val)) [[unlikely]]
			{
				helper.post_err(position,
				                escape_error_not_unicode_scalar_value,
				                get_esc_not_scalar_err_msg(u8"U"_em, seq_to_err_msg_str(), parse_val, false));
				rt_flag = true;
			}
			if (rt_flag) [[unlikely]]
			{
		...
		default:
			if (lexer_utility::is_oct(c))
			{
				...
				if (!unicode::is_scalar_value(parse_val)) [[unlikely]]
				{
					helper.post_err(position,
					                escape_error_not_unicode_scalar_value,
					                get_esc_not_scalar_err_msg(u8""_em, seq_to_err_msg_str(), parse_val, false));
					rt_flag = true;
				}
				if (rt_flag) [[unlikely]]
				{
		...
		helper.post_err(position, escape_error_unknown_escape_sequence, to_raw_err_str(::std::move(err_msg)));
		return false;
	}
	...
	inline bool get_raw_string_opening_d_seq(unicode::ucpstring& out_sequence,
		                                                   auto& begin,
		                                                   auto  end,
		                                                   bool  is_raw_id_s,
		                                                   auto  helper)
	{
		...
		while (begin != end)
		{
			...
			else
			{
				...
				{
					helper.post_err(helper.get_pos(),
		...
		helper.post_err(position, d_seq_error_cannot_find_end_sign, raw_err{ code_convert<err_ms>(out_sequence), is_raw_id_s });
	...
	inline bool get_raw_string_closing_d_seq(unicode::ucpstring_view in_sequence,  // delimiter sequence for reference
	...
	inline void process_hex_fp_errors(dfa_state_code c, ::std::string& num_seq, source_position pos, auto helper)
	{
		...
		case hex_fp_dec_part_with_quote:  // <<< ERROR STATE
		{
			...
			using enum raw_error_message_type::number_type;
			helper.post_err(pos, lex_ec::number_cannot_end_with_separator, raw_err{ num_seq_to_ems(), hex_floating });
			[[fallthrough]];
		}
		...
		case hex_fp_exp_first:         // <<< ERROR STATE
			helper.post_err(pos, lex_ec::fp_hex_expect_exponent, to_raw_err_str(num_seq_to_ems()));
			switch (c)
			{
		...
		case hex_fp_seq_start_with_0x_dot: // <<< ERROR STATE
			helper.post_err(pos, lex_ec::fp_hex_expect_decimal_part, to_raw_err_str(num_seq_to_ems()));
			helper.post_err(pos, lex_ec::fp_hex_expect_exponent, to_raw_err_str(num_seq_to_ems()));
			num_seq += "0P0"s;
			break;
		default:
	...
	inline void process_string_like_error(dfa_state_code          dfa_state_c,
		                                  source_position         position,
		                                  error_msg_string        err_s,
		                                  unicode::ucpstring_view d_seq,
		                                  auto                    helper)
	{
		using lex_ec = lexical_error_code;
		using enum dfa_state_code;
		...
		case identifier_string:
			helper.post_err(position, lex_ec::identifier_string_missing_terminating_character, to_raw_err_str(::std::move(err_s)));
			break;
		case string_opened:
		case string:
			helper.post_err(position, lex_ec::string_missing_terminating_character, to_raw_err_str(::std::move(err_s)));
			break;
		case identifier_raw_string:
			helper.post_err(position, lex_ec::identifier_raw_string_missing_terminating_sequence, make_missing_d_seq_err(true));
			break;
		case raw_string:
			helper.post_err(position, lex_ec::raw_string_missing_terminating_sequence, make_missing_d_seq_err(false));
			break;
		default:
			break;
		}
	...
	inline void process_char_missing_quote_error(source_position position, error_msg_string err_s, auto helper)
	{
		using lex_ec = lexical_error_code;
		helper.post_err(position, lex_ec::character_missing_terminating_character, to_raw_err_str(::std::move(err_s)));
	}

	inline void process_dec_seq_error(const dfa_state_code code, source_position position, error_msg_string error_str, auto helper)
	{
		using lex_ec = lexical_error_code;
		using enum dfa_state_code;
		using raw_err = raw_error_message_type::number_end_with_separator;
		...
		{
			helper.post_err(position, lex_ec::invalid_octal_number, to_raw_err_str(::std::move(error_str)));
		}
		else if (code == dec_seq_start_with_0_with_quote)
		{
			using enum raw_error_message_type::number_type;
			helper.post_err(position, lex_ec::number_cannot_end_with_separator, raw_err{ ::std::move(error_str), oct_integer });
		}
		else if (code == dec_seq_with_quote)
		{
			using enum raw_error_message_type::number_type;
			helper.post_err(position, lex_ec::number_cannot_end_with_separator, raw_err{ ::std::move(error_str), dec_integer });
		}
	}
	...
	template <concepts::function_package_for_lexer function_package>
	class lexer_helper
	{
	public:
		explicit lexer_helper(function_package& function_pkg) : func_pkg{ &function_pkg } {}
...
namespace pdn
{
	...
	public:
		token<char_t> get_token(auto&& begin, auto end)
		{
			...
			source_position            position{ helper.get_pos() };
			...
			while (begin != end)
			{
				...
				if (!unicode::is_scalar_value(c))
				{
					...
					helper.post_err(helper.get_pos(), lex_ec::not_unicode_scalar_value, raw_err{ c });
					++begin;
					continue;
				}
				...
				auto update_token_pos = [&]()
				{
					...
					if (detail::is_non_token_state(prev) && detail::is_token_state(next))
					{
						position = helper.get_pos();
					}
				};
				...
				case unacceptable_character:
				{
					...
					helper.post_err(helper.get_pos(),
					                ...
					                detail::to_raw_err_str(code_convert<err_ms>(view)));
					break;
				}
				case infinity:
				{
				...
				case identifier_string_with_LF:
				{
					...
					helper.post_err(position, lex_ec::identifier_string_terminated_by_LF, detail::to_raw_err_str(code_convert<err_ms>(text)));
					goto label_update_dfa_state;
				}
				case string_with_LF:
				{
					new_dfa_state = dfa_state_objects::string_closed;
					helper.post_err(position, lex_ec::string_literal_terminated_by_LF, detail::to_raw_err_str(code_convert<err_ms>(text)));
					goto label_update_dfa_state;
				}
				case character_with_LF:
				{
					new_dfa_state = dfa_state_objects::character_closed;
					helper.post_err(position, lex_ec::character_literal_terminated_by_LF, detail::to_raw_err_str(code_convert<err_ms>(text)));
					goto label_update_dfa_state;
				}
				case start:
				case line_comment:
				...
				case identifier_string_escape:
					...
					if (detail::get_escape(c, begin, end, true, helper))
					{
						append(text, c);
					}
					goto label_update_dfa_state;
				case string_escape:
					...
					if (detail::get_escape(c, begin, end, false, helper))
					{
						append(text, c);
					}
					goto label_update_dfa_state;
				...
				case raw_string_d_seq_opened:
					...
					detail::get_raw_string_opening_d_seq(open_d_seq, begin, end, false, helper);
				...
				case raw_string_received_right_parentheses:
				{
					...
					++begin;
					if (detail::get_raw_string_closing_d_seq(open_d_seq, close_d_seq, begin, end, U'"'))
					{
				...
				case identifier_raw_string_d_seq_opened:
					...
					// @`d_seq()d_seq`
					//  ^ -> to first d sequence character or '('
					detail::get_raw_string_opening_d_seq(open_d_seq, begin, end, true, helper);
				...
				case hex_fp_exp:
					...
					{
						helper.post_err(helper.get_pos(),
						                lex_ec::more_than_one_separators_between_numbers,
						                detail::to_raw_err_str(num_seq_to_ems()));
					}
		...
		label_out_of_loop:
			...
			case block_comment:
			case block_comment_closing:
				helper.post_err(position, lexical_error_code::unterminated_block_comment, {});
				break;
			case nested_block_comment:
			case nested_block_comment_nesting:
			case nested_block_comment_nested:
			case nested_block_comment_closing:
			case nested_block_comment_closed:
				helper.post_err(position, lex_ec::unterminated_nested_block_comment, {});
				break;
			case identifier_string_opened:
			case identifier_string:
			case string_opened:
			...
			case identifier_raw_string:
			case raw_string:
				detail::process_string_like_error(dfa_state.state_code, position, detail::to_err_ms<char_t>(text), open_d_seq, helper);
				[[fallthrough]];
				// ERROR STATES ^^^
			case identifier:
			...
			case character:        // <<< ERROR STATE
				detail::process_char_missing_quote_error(position, detail::to_err_ms<char_t>(text), helper);
				[[fallthrough]];
			case character_closed:
				result.value = detail::process_char_literal<char_t>(text, position, helper);
				break;
			case dec_seq_start_with_0:             // <<< ERROR STATE
			case dec_seq_start_with_0_with_quote:  // <<< ERROR STATE
			case dec_seq_with_quote:               // <<< ERROR STATE
				detail::process_dec_seq_error(dfa_state.state_code, position, num_seq_to_ems(), helper);
				[[fallthrough]];
			case dec_seq:
			{
				...
				detail::to_appropriate_int_type<char_t>(result.value, integer_value);
				if (!detail::check_from_chars_succeed(from_chars_r, n_seq.data() + n_seq.size())) [[unlikely]]
				{
					using enum raw_error_message_type::number_type;
					detail::process_from_chars_error(from_chars_r, n_seq, position, dec_integer, false, helper);
				}
				break;
			}
			...
			case fp_exp_with_quote:       // <<< ERROR STATE
				...
				{
					helper.post_err(position, lex_ec::fp_dec_expect_exponent, detail::to_raw_err_str(num_seq_to_ems()));
				...
				else if (dfa_state.state_code == fp_dec_part_with_quote || dfa_state.state_code == fp_exp_with_quote)
				{
					using raw_err = raw_error_message_type::number_end_with_separator;
					using enum raw_error_message_type::number_type;
					helper.post_err(position, lex_ec::number_cannot_end_with_separator, raw_err{ num_seq_to_ems(), dec_floating });
				}
				[[fallthrough]];
			case fp_dec_part_first_after_dec_with_dot:
			case fp_dec_part:
			case fp_exp:
			{
				...
				{
					using enum raw_error_message_type::number_type;
					detail::process_from_chars_error(from_chars_r, n_seq, position, dec_floating, false, helper);
				}
				break;
			}
			case oct_seq_with_quote:  // <<< ERROR STATE
			{
				using raw_err = raw_error_message_type::number_end_with_separator;
				using enum raw_error_message_type::number_type;
				helper.post_err(position, lex_ec::number_cannot_end_with_separator, raw_err{ num_seq_to_ems(), oct_integer });
				[[fallthrough]];
			}
			case zero:
			case oct_seq:
			{
				...
				{
					using enum raw_error_message_type::number_type;
					detail::process_from_chars_error(from_chars_r, n_seq, position, oct_integer, false, helper);
				}
			}
			break;
			...
			case bin_seq_with_quote:  // <<< ERROR STATE
				...
				{
					helper.post_err(position, lex_ec::bin_prefix_expect_bin_number_sequence, detail::to_raw_err_str(num_seq_to_ems()));
					number_sequence += '0';
				}
				else if (dfa_state.state_code == bin_seq_with_quote)
				{
					using raw_err = raw_error_message_type::number_end_with_separator;
					using enum raw_error_message_type::number_type;
					helper.post_err(position, lex_ec::number_cannot_end_with_separator, raw_err{ num_seq_to_ems(), bin_integer });
				}
				[[fallthrough]];
			case bin_seq:
			{
				...
				{
					using enum raw_error_message_type::number_type;
					detail::process_from_chars_error(from_chars_r, n_seq, position, bin_integer, false, helper);
				}
				break;
			}
			...
			case hex_seq_with_quote:  // <<< ERROR STATE
				...
				{
					helper.post_err(position, lex_ec::hex_prefix_expect_hex_number_sequence, detail::to_raw_err_str(num_seq_to_ems()));
					number_sequence += '0';
				}
				else if (dfa_state.state_code == hex_seq_with_quote)
				{
					using raw_err = raw_error_message_type::number_end_with_separator;
					using enum raw_error_message_type::number_type;
					helper.post_err(position, lex_ec::number_cannot_end_with_separator, raw_err{ num_seq_to_ems(), hex_integer });
				}
				[[fallthrough]];
			case hex_seq:
			{
				...
				{
					using enum raw_error_message_type::number_type;
					detail::process_from_chars_error(from_chars_r, n_seq, position, hex_integer, false, helper);
				}
				break;
			}
			case hex_fp_dec_part_with_quote:               // <<< ERROR STATE
			...
			case hex_fp_exp_with_quote:                    // <<< ERROR STATE
			case hex_fp_seq_start_with_0x_dot:             // <<< ERROR STATE
				detail::process_hex_fp_errors(dfa_state.state_code, number_sequence, position, helper);
				[[fallthrough]];
			case hex_fp_exp:
			{
				assert(detail::check_0x_prefix(number_sequence) && "hexadecimal literal without prefix");
				...
				{
					using enum raw_error_message_type::number_type;
					detail::process_from_chars_error(from_chars_r, n_seq, position, hex_floating, false, helper);
				}
				break;
			}
			default:
		...
		explicit lexer(function_package& function_pkg) : helper{ function_pkg } {}
	private:
		detail::lexer_helper<function_package> helper{};
	};
}
