...
namespace pdn_test
{
	...
	struct my_function_package
	{
		...
		{
#ifdef _WIN32
			if (err_count == 0 && pdn_test::g_use_stdout)
			{
				::std::cout << "Do you want to chcp 65001 before print error messages(Y/n default = n): ";
				::std::string ans{};
		...
		for (; ; )
		{
			...
			if (is_iden(tk))
			{
				...
				else if (cmd_str == ct::chcp) // for test on Windows
				{
					...
					::std::system("chcp 65001");
#else
					::std::cout << "this command only for Windows.\n";
#endif
				}
				else
				{
	...
	void test(const std::string& filename, std::ostream& out, std::ostream& log, bool to_play = false)
	{
		...
		{
			print<char_t>(out, dom, ::std::numeric_limits<::std::size_t>::max());
		}
	}
}
...
int main(int argc, const char* argv[])
{
	...
	pdn_test::g_use_stdout = is_using_stdout || to_play;
	std::ostream& out = pdn_test::g_use_stdout ? std::cout : out_file;
	std::ostream& log = pdn_test::g_use_stdout ? std::cout : log_file;
