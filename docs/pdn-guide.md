...
在 `spdn` 中，数据都有自己的类型，类型决定了数据的形式和取值范围。最常用的，莫过于数、字符串、列表和对象。  
我们可以为实体指定类型：`实体名称 : 类型 数据的表示`，如 `num: int 123`，为列表的元素指定类型使用 `类型 : 数据的表示`，如 `list [int: 123, float: 1.0]`。  
警告，如果没有必要，不要为实体指定类型！

```spdn
an_i8_number: i8 127 // 还行，我想明确表示数据的类型是 i8，范围是 -128 到 127
a_regular_number: int 123 // 不好，这个类型指定完全是多余的！
a_number_less_than_10: i8 9 // 不好，除非你有其他使用 i8 的理由。更具体的约束不应该由 spdn 来做，而且这个类型指定对“小于10”这个约束也完全没有作用
object: object { num: 123 } // 不好！{ num: 123 } 不是对象还能是什么？这个类型指定完全是多余的！
...
可以看到像某些编程语言一样冗杂的有关数字的类型，它们很可能是一种设计失误。设计它们，是为了能更好的对应 C++ 的整数和浮点数类型，这些整数类型均是 C++ 标准库 `cstdint` 中 `int8_t` 到 `int64_t`、`uint8_t` 到 `uint64_t` 的别名。并且，给用户选择的权力，如指定一个实体是 `f32` 类型的：`num:f32 1`，如果不使用类型指定，那么 `num` 的类型就是 `int`，而不是 `f32`。  
...
我们在 `spdn` 中写下的整数，绝大部分是 `int` 类型的，如果我们写下的数字超过了 `int` 类型能表示的范围，解析器会为它选择一个合适的整数类型，如果没有能表示这个整数的类型，那么就会产生错误：`1` 的类型是 `int`，`0xffff'ffff'ffff'ffff` 的类型是 `u64`，`0x1'0000'0000'0000'0000` Oops! 现在连 `u64` 都无法满足，只能报告错误了！  
其实像这样的情况应该换用浮点数表示了，`0x1p64`，这里使用的是十六进制科学计数法表示。还记得我们说过 `spdn` 参考了 C++ 的字面量词法吗？`spdn` 支持[二进制、八进制、十进制和十六进制的整数](https://zh.cppreference.com/w/cpp/language/integer_literal "cppreference")，以及[十进制、十进制科学计数法、十六进制科学计数法的浮点数](https://zh.cppreference.com/w/cpp/language/floating_literal "cppreference")。  
在 `spdn` 里浮点数都是 `f64` 类型的。  

#### 布尔类型

`spdn` 的布尔类型，和 C++ 的 `bool` 类型对应。  

```spdn
bool_1: boolean 1       // 非零值被转换为布尔值 true
bool_2: bool    123.456 // 非零值被转换为布尔值 true，bool 和 boolean 等价
...
由于有的符号或形状可以由多个码点组合表示，所以有些看起来是一个字符的内容实际上不能由 `spdn` 的字符表示：`'😀'` 合法，`'🧑‍⚕️'` 不合法。  
